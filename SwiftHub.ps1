# ==============================================================================
# 🌌 SWIFTHUB CORE v7.0 - GOD TIER (PROFILES, DEBLOAT, RESTORE & CUSTOM UI)
# ==============================================================================
...
$global:i18n = @{
    "TR" = @{
        "AppTitle" = "SWIFTHUB TITAN v7.0"; "AppSub" = "GOD TIER SISTEM MERKEZI";
        "MenuDash" = "🏠 Kontrol Paneli"; "MenuApps" = "📦 Uygulamalar"; "MenuTweaks" = "🛠️ Ince Ayarlar";
        "MenuFeatures" = "⚙️ Windows Ozellikleri"; "MenuNet" = "🌐 Ag ve Guvenlik"; "MenuFixes" = "🩹 Sistem Onarimi";
        "BtnLang" = "🌐 EN"; "StatusWait" = "Sistem hazir. Mimarinin keyfini cikarin...";
        "DashWelcome" = "SwiftHub v7.0'a Hos Geldiniz!"; "DashSub" = "Kusursuz hiz, sifir iz ve devasa ozellikler. Sol menuden operasyona baslayin.";
    ...
    "EN" = @{
        "AppTitle" = "SWIFTHUB TITAN v7.0"; "AppSub" = "GOD TIER SYSTEM CENTER";
        "MenuDash" = "🏠 Dashboard"; "MenuApps" = "📦 Applications"; "MenuTweaks" = "🛠️ System Tweaks";
        "MenuFeatures" = "⚙️ Windows Features"; "MenuNet" = "🌐 Network & Security"; "MenuFixes" = "🩹 System Fixes";
        "BtnLang" = "🌐 TR"; "StatusWait" = "System ready. Enjoy the architecture...";
        "DashWelcome" = "Welcome to SwiftHub v7.0!"; "DashSub" = "Flawless speed, zero footprint, massive features. Start from the sidebar.";
...
# --- 2. XAML TASARIMI (CUSTOM SCROLLBAR & FULL ARSENAL) ---
[xml]$xaml = @"
                    ...
                    <ControlTemplate TargetType="ScrollBar">
                        <Grid Background="{TemplateBinding Background}">
                            <Track Name="PART_Track" IsDirectionReversed="true">
                                <Track.Thumb>
                                    <Thumb Background="#1F222B" BorderThickness="0">
                                        <Thumb.Template>
            ...
            <Grid Grid.Column="1" Margin="30">
                ...
                <Grid Name="PageDash" Visibility="Visible">
                    <StackPanel VerticalAlignment="Center" HorizontalAlignment="Center">
                        <TextBlock Name="TxtDashWelcome" Text="SwiftHub v7.0'a Hos Geldiniz!" FontSize="38" FontWeight="Black" Foreground="White" TextAlignment="Center" Margin="0,0,0,10"/>
                ...
                <Grid Name="PageApps" Visibility="Hidden">
                    ...
                    <Grid Grid.Row="1" Margin="0,15,0,0">
                        ...
                        <Button Name="BtnDebloat" Content="☢️ Nükleer Debloat (Çöpleri Sil)" Style="{StaticResource ActionBtn}" Background="#FF3B30" Grid.Column="3"/>
                    </Grid>
                </Grid>
                ...
                <Grid Name="PageFeatures" Visibility="Hidden">
                    <StackPanel>
                        <TextBlock Text="⚙️ WINDOWS ÖZELLİKLERİ" FontSize="26" FontWeight="Black" Foreground="White" Margin="0,0,0,20"/>
                        ...
                        <TextBlock Text="Not: Bu ozelliklerin aktiflesmesi bilgisayarinizin yeniden baslamasini gerektirebilir." Foreground="#8A8D93" Margin="5,10,0,0"/>
                    </StackPanel>
                </Grid>

                <Grid Name="PageNet" Visibility="Hidden">
                    <StackPanel>
                        <TextBlock Text="🌐 AĞ VE SİBER GÜVENLİK" FontSize="26" FontWeight="Black" Foreground="White" Margin="0,0,0,20"/>
                ...
                <Grid Name="PageFixes" Visibility="Hidden">
                    <StackPanel>
                        <TextBlock Text="🩹 SİSTEM ONARIMI" FontSize="26" FontWeight="Black" Foreground="White" Margin="0,0,0,20"/>
                        ...
                        <Button Name="BtnFixSpooler" Content="Yazdirma Biriktiricisini Onar (Print Spooler)" Style="{StaticResource ActionBtn}"/>
                        <Border Background="#13151A" BorderBrush="#1F222B" BorderThickness="1" CornerRadius="6" Height="150" Margin="5,10,5,0">
...
$BtnFixSFC=$window.FindName("BtnFixSFC"); $BtnFixWU=$window.FindName("BtnFixWU"); $BtnFixTemp=$window.FindName("BtnFixTemp"); $BtnFixSpooler=$window.FindName("BtnFixSpooler")
...
# YENI BUTONLAR (PROFIL, DEBLOAT, RESTORE)
...
$BtnTweakExport=$window.FindName("BtnTweakExport"); $BtnTweakImport=$window.FindName("BtnTweakImport"); $BtnRestore=$window.FindName("BtnRestore")

$global:AppHeaders=@(); $global:AppItems=@(); $global:TweakHeaders=@(); $global:TweakItems=@()

# ==============================================================================
# 🚀 3. DINAMIK JSON MOTORU (SONSUZ KAPASITE)
# ==============================================================================
try {
...
function Update-Language {
    ...
    $BtnDebloat.Content = $d["BtnDebloat"]; $BtnRestore.Content = $d["BtnRestore"]
...
$BtnInstallApps.Add_Click({
    $TxtStatus.Text = "[*] Programlar arka planda kuruluyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render)
    ...
    $TxtStatus.Text = "[+] $c program kuruldu!"
})
$BtnApplyTweaks.Add_Click({
    $TxtStatus.Text = "[*] Ayarlar enjekte ediliyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render)
    $c = 0; foreach ($i in $global:TweakItems) { if ($i.CheckBox.IsChecked) { try { Invoke-Expression $i.Script; $c++ } catch {} } }
    $TxtStatus.Text = "[+] $c ayar uygulandi!"
})

# IMPORT / EXPORT (PROFILLER)
$BtnAppExport.Add_Click({
    $dlg = New-Object Microsoft.Win32.SaveFileDialog; $dlg.Filter = "JSON Profile (*.json)|*.json"; $dlg.FileName = "SwiftHub_AppsProfile.json"
    if ($dlg.ShowDialog() -eq $true) { $selected = $global:AppItems | Where { $_.CheckBox.IsChecked } | Select-Object Id; $selected | ConvertTo-Json -Depth 10 | Out-File $dlg.FileName -Encoding UTF8; $TxtStatus.Text = "[+] Uygulama profili kaydedildi!" }
})
$BtnAppImport.Add_Click({
    $dlg = New-Object Microsoft.Win32.OpenFileDialog; $dlg.Filter = "JSON Profile (*.json)|*.json"
    if ($dlg.ShowDialog() -eq $true) { 
        $json = Get-Content $dlg.FileName -Raw | ConvertFrom-Json
        foreach ($i in $global:AppItems) { $i.CheckBox.IsChecked = $false; foreach ($j in $json) { if ($i.Id -eq $j.Id) { $i.CheckBox.IsChecked = $true } } }
        $TxtStatus.Text = "[+] Uygulama profili yuklendi!" 
    }
})
$BtnTweakExport.Add_Click({
    $dlg = New-Object Microsoft.Win32.SaveFileDialog; $dlg.Filter = "JSON Profile (*.json)|*.json"; $dlg.FileName = "SwiftHub_TweaksProfile.json"
    if ($dlg.ShowDialog() -eq $true) { $selected = $global:TweakItems | Where { $_.CheckBox.IsChecked } | Select-Object Script; $selected | ConvertTo-Json -Depth 10 | Out-File $dlg.FileName -Encoding UTF8; $TxtStatus.Text = "[+] Ayar profili kaydedildi!" }
})
$BtnTweakImport.Add_Click({
    $dlg = New-Object Microsoft.Win32.OpenFileDialog; $dlg.Filter = "JSON Profile (*.json)|*.json"
    if ($dlg.ShowDialog() -eq $true) { 
        $json = Get-Content $dlg.FileName -Raw | ConvertFrom-Json
        foreach ($i in $global:TweakItems) { $i.CheckBox.IsChecked = $false; foreach ($j in $json) { if ($i.Script -eq $j.Script) { $i.CheckBox.IsChecked = $true } } }
        $TxtStatus.Text = "[+] Ayar profili yuklendi!" 
    }
})

# DEBLOAT & RESTORE POINT
$BtnRestore.Add_Click({
    $TxtStatus.Text = "[*] Sistem Geri Yukleme Noktasi olusturuluyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render)
    Enable-ComputerRestore -Drive "C:\" -ErrorAction SilentlyContinue | Out-Null
    Checkpoint-Computer -Description "SwiftHub Titan Backup" -RestorePointType "MODIFY_SETTINGS" -ErrorAction SilentlyContinue | Out-Null
    $TxtStatus.Text = "[+] Guvenlik Aği aktif! Geri yukleme noktasi basariyla olusturuldu."
})
$BtnDebloat.Add_Click({
    $TxtStatus.Text = "[☢️] Nukleer Debloat basladi! Gereksiz Windows uygulamalari siliniyor (1-2 dakika surebilir)..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render)
    ...
    $c = 0; foreach ($app in $bloatware) { try { Get-AppxPackage -Name "*$app*" -AllUsers -ErrorAction SilentlyContinue | Remove-AppxPackage -AllUsers -ErrorAction SilentlyContinue; $c++ } catch {} }
    $TxtStatus.Text = "[+] Debloat Tamamlandi! $c adet çöp paket sistemden köküne kadar kazındı."
})

# EXPANDED SYSINFO (GOD MODE)
$BtnAnalyze.Add_Click({
    $TxtSysInfo.Text = "Derin Telemetri Okunuyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render)
    $os = Get-CimInstance Win32_OperatingSystem -ErrorAction SilentlyContinue; $cpu = Get-CimInstance Win32_Processor -ErrorAction SilentlyContinue
    $mb = Get-CimInstance Win32_BaseBoard -ErrorAction SilentlyContinue; $bios = Get-CimInstance Win32_BIOS -ErrorAction SilentlyContinue
    $ram = Get-CimInstance Win32_PhysicalMemory -ErrorAction SilentlyContinue; $net = Get-CimInstance Win32_NetworkAdapterConfiguration | Where-Object IPEnabled -eq $true
    
    $info = "[ISLETIM SISTEMI]`nModel     : $($os.Caption) $($os.OSArchitecture)`nVersiyon  : $($os.Version) (Build $($os.BuildNumber))`nCalisma   : $(([math]::Round(((Get-Date) - $os.LastBootUpTime).TotalHours, 1))) Saat`n"
    $info += "`n[ANAKART & BIOS]`nModel     : $($mb.Manufacturer) $($mb.Product)`nBIOS Srm  : $($bios.SMBIOSBIOSVersion) ($($bios.ReleaseDate.ToString('dd.MM.yyyy')))`n"
    $info += "`n[ISLEMCI (CPU)]`nModel     : $($cpu[0].Name)`nCekirdek  : $($cpu[0].NumberOfCores) Core / $($cpu[0].NumberOfLogicalProcessors) Threads`n"
    $info += "`n[BELLEK (RAM)]`nKapasite  : $([math]::Round(($ram | Measure-Object Capacity -Sum).Sum / 1GB, 2)) GB`nAktif İslem: $($os.NumberOfProcesses) Process`n"
    $info += "`n[AĞ ADRESLERI (NETWORK)]`n"
    foreach ($n in $net) { $info += "Bagdastirici: $($n.Description)`nIPv4 Adresi : $($n.IPAddress[0])`nMAC Adresi  : $($n.MACAddress)`n`n" }
...
$BtnFixSFC.Add_Click({ $TxtFixLog.Text = "[*] DISM & SFC Onarimi (Uzun surebilir)..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render); Start-Process "cmd.exe" -ArgumentList "/c DISM /Online /Cleanup-Image /RestoreHealth & sfc /scannow" -Wait -NoNewWindow; $TxtFixLog.Text = "[+] Onarim bitti!" })
$BtnFixWU.Add_Click({ $TxtFixLog.Text = "[*] Windows Update onbellegi siliniyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render); Stop-Service wuauserv -Force -ErrorAction SilentlyContinue; Stop-Service bits -Force -ErrorAction SilentlyContinue; Remove-Item "$env:windir\SoftwareDistribution\Download\*" -Recurse -Force -ErrorAction SilentlyContinue; Start-Service wuauserv -ErrorAction SilentlyContinue; Start-Service bits -ErrorAction SilentlyContinue; $TxtFixLog.Text = "[+] Update onbellegi sifirlandi!" })
$BtnFixTemp.Add_Click({ $TxtFixLog.Text = "[*] Temp temizleniyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render); Remove-Item "$env:TEMP\*" -Recurse -Force -ErrorAction SilentlyContinue; Remove-Item "$env:windir\Temp\*" -Recurse -Force -ErrorAction SilentlyContinue; $TxtFixLog.Text = "[+] Temp temizlendi!" })
$BtnFixSpooler.Add_Click({ $TxtFixLog.Text = "[*] Yazici servisi onariliyor..."; $window.Dispatcher.Invoke([Action]{}, [Windows.Threading.DispatcherPriority]::Render); Stop-Service Spooler -Force -ErrorAction SilentlyContinue; Remove-Item "$env:windir\System32\spool\PRINTERS\*" -Force -ErrorAction SilentlyContinue; Start-Service Spooler -ErrorAction SilentlyContinue; $TxtFixLog.Text = "[+] Yazici servisi onarildi!" })
...
$BtnFeatNet2.Add_Click({ Start-Process "dism.exe" -ArgumentList "/online /enable-feature /featurename:NetFx3 /all /norestart" -Wait -NoNewWindow; $TxtStatus.Text="[+] .NET 3.5 Kuruldu. Yeniden baslatin." })
